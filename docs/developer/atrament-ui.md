...
To create a new markup tag, you need to make a Preact component which will render it.

For example, let's create a `[list]` markup tag, which will render `[list]a,b,c[/list]` into `<ul><li>a</li><li>b</li><li>c</li></ul>`.

1. Create folder `list` in `src/components/markup`. Folder name does not matter.
...
- `content` is the text inside of a tag. Don't forget to render its parts with `markup()`, as in example above, otherwise other tags in it will not be processed. 
...
By default, Atrament uses `inklecate` for Windows, Linux, and MacOS. If `inklecate` is not available for the platform, inkjs compiler will be used.
To force using InkJS compiler everywhere, add this line to `atrament.config.json`:
