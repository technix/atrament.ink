...
Всі теги розмітки мають атрибут `class`, за допомогою якого можна застосувати вказаний CSS-клас до елементів розмітки:

```
[block width=50% class="custom_css_class"]Текст[/block]
```

Додавання власних CSS-класів в проєкт на Атраменті описане в розділі "[Оформлення](atrament-extras/customization.md)".

## Розташування

### Розділювач

Ви можете розділити абзаци тексту горизонтальною лінією за допомогою тега розмітки `[---]`.
...
| Атрибут | Опис                |
| :------ | :------------------ |
| `width` | Довжина розділювача (за замовчуванням це "60%"). Може бути задана у відсотках (дуже рекомендуються) або інших одиницях CSS. |
...
| `align=left` | Встановити вирівнювання тексту: ліворуч (`left`), праворуч (`right`) або по центру (`center`, встановлено за замовчуванням). |
| `border=none` | Встановити рамки заголовка: обидві (`both`, встановлено за замовчуванням), зверху (`top`), внизу (`bottom`), або ж прибрати їх (`none`). |
...
| `align=left` | Встановити вирівнювання тексту: ліворуч (`left`), праворуч (`right`) або по центру (`center`). |
...
```
Цей текст [font family=Courier New]використовує моноширинний шрифт[/font].
```
...
| `border=false` | Якщо встановлений, рамка індикатору не відображається. |
| `display=thin` | Якщо встановлений, товщина індикатора буде зменшена. |
| `solid=true` | Якщо встановлений, шкала індикатора буде менш прозорою. |
...
Тег розмітки `[link]` робить посилання на варіанти вибору або виклик функції Ink. Зазвичай це використовується в [гіпертекстовому](atrament-basics/story.md#гіпертекст) режимі.

```
=== continue_story
[link to=Продовжити]Натисніть для продовження[/link]

+ [Продовжити]
    -> continue_knot
```

Тег розмітки `[link]` має такі атрибути:

| Атрибут   | Опис                |
| :-------- | :------------------ |
| `to="Вибір 1"` | Текст варіанту вибору, який має бути вибраний при натисканні посилання. |
| `onclick=inkFunction` | Назва функції, яка викликається при натисканні посилання. Функція не може приймати параметри. |
| `display=modal` | Якщо встановлений, натискання посилання відображає модальне вікно оверлею замість повноекранного. |
...
```
Завітайте на сайт [url href=http:\/\/atrament.ink/]Атраменту[/url].
```
